    ...
    <?php else : ?>
      <form id="commentform" method="POST" action="<?php echo esc_url( home_url() ); ?>/wp-comments-post.php">
        <?php if ( is_user_logged_in() ) : ?>
          <p>Logged in as <a
              href="<?php echo esc_url( home_url( '/' ) ); ?>wp-admin/profile.php"><?php echo $user_identity; ?></a>. <a
              href="<?php echo wp_logout_url( get_permalink() ); ?>" title="Log out of this account">Log out &raquo;</a>
          </p>
        <?php else : ?>
          <div>
            <label for="author">Name <?php if ( $req ) {
              ...
              <input type="text" name="author" id="author" value="<?php echo esc_attr( $comment_author ); ?>" size="22"
